  ...
  useEffect(() => {
    ...
    if (products) {
      const indexProduct = products.findIndex(
        (p: ProductType) => p._id === id_product
      );
    ...
    const fetchGetProduct = async () => {
      if (id_product) {
        ...
        const findIndex = resProducts.findIndex(
          (p: ProductType) => p._id === id_product
        );
    ...
    <div id="ProductDetailsPage">
      ...
      <section className="product_details">
        ...
        <footer className="btn_footer">
          {isOrdered ? (
            <button onClick={removeUserFromProduct}>בטל הצטרפות</button>
          ) : (
            <button onClick={addUserToProduct}>הצטרף כעת להזמנה</button>
          )}
